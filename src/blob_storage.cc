...
namespace rocksdb
{
  namespace titandb
  {
    ...
    void BlobStorage::PrintFileStates()
    {
      ...
      std::vector<std::vector<double>> discardable_ratio_level(cf_options_.num_levels);
      ...
      for (auto &file : files_)
      {
        ...
        numFile[level]++;
        discardable_ratio_level[level].push_back(file.second->GetDiscardableRatio());
        switch (file.second->file_state())
        {
        case BlobFileMeta::FileState::kObsolete:
          numObsolete[level]++;
          ROCKS_LOG_INFO(db_options_.info_log, "Obsolete file: %d", file.second->file_number());
          break;
